...
# Fixed PCA setting used in the final report
N_PCS = 15
...
def run_pca_grid_search(raw_features, feature_cols, signal_name, n_pcs_grid=(2, 4, 5, 10, 15, 20, 25)):
    grid_fold_metrics = []
    grid_predictions = []
    grid_summary_rows = []
...
if __name__ == "__main__":
    ...
    if raw_features.empty:
        raise FileNotFoundError(
            f"No dataset found at {DATASET_ROOT}. Put the dataset in ./data/dataset "
            "or set PROJECT2_DATA_DIR to the data folder."
        )
    ...
    for signal_name, feature_cols in signal_feature_sets.items():
        print(f"\nRunning LOSO for {signal_name} with fixed PCs={N_PCS}")
        ...
        if fold_metrics.empty or preds.empty:
            raise RuntimeError(f"No valid folds produced for signal set: {signal_name}")

        final_predictions[signal_name] = preds
        final_fold_metrics[signal_name] = fold_metrics
        final_ablation_results[signal_name] = compute_summary_stats(fold_metrics, preds)
    ...
    print("\nFinal Ablation Summary Using Fixed PCA Components")
    ...
    summary_df.to_csv("temporal_ablation_summary_fixed_pca.csv", index=False)
    print("\nSaved: temporal_ablation_summary_fixed_pca.csv")
    ...
    axes[0].set_title(f"OCSVM — all signals, PCs={N_PCS}")
    ...
    axes[1].set_title(f"GMM — all signals, PCs={N_PCS}")

    tn_g, fp_g, fn_g, tp_g = cm_gmm.ravel()

    fig.suptitle("LOSO Confusion Matrices: Zero-Shot Stress Detection", fontsize=14, y=1.02)
    plt.tight_layout()
    plt.savefig("confusion_matrices_fixed_pca.png", dpi=150, bbox_inches="tight")
    print("\nSaved: confusion_matrices_fixed_pca.png")
    plt.show()
    ...
    axes[0, 0].set_title(f"OCSVM — HR Only, PCs={N_PCS}")
    ...
    axes[0, 1].set_title(f"GMM — HR Only, PCs={N_PCS}")
    ...
    axes[1, 0].set_title(f"OCSVM — EDA Only, PCs={N_PCS}")
    ...
    axes[1, 1].set_title(f"GMM — EDA Only, PCs={N_PCS}")
    ...
    axes[2, 0].set_title(f"OCSVM — TEMP Only, PCs={N_PCS}")
    ...
    axes[2, 1].set_title(f"GMM — TEMP Only, PCs={N_PCS}")
    ...
    axes[3, 0].set_title(f"OCSVM — BVP Only, PCs={N_PCS}")
    ...
    axes[3, 1].set_title(f"GMM — BVP Only, PCs={N_PCS}")

    fig.suptitle("LOSO Confusion Matrices: Signal Ablation with Fixed PCA", fontsize=14, y=0.995)
    plt.tight_layout()
    plt.savefig("confusion_matrices_signal_ablation_fixed_pca.png", dpi=150, bbox_inches="tight")
    print("\nSaved: confusion_matrices_signal_ablation_fixed_pca.png")
    plt.show()
    ...
    ax.set_title("Signal Ablation: Raw Temporal Windows with Fixed PCA", fontsize=14)
    ax.set_xticks(x)
    ax.set_xticklabels([f"{s}\nPCs={N_PCS}" for s in signals])
    ax.legend(fontsize=10)
    ax.set_ylim(0.3, 1.0)
    ax.grid(axis="y", alpha=0.3)

    plt.tight_layout()
    plt.savefig("temporal_ablation_barplot_fixed_pca.png", dpi=150)
    print("\nSaved: temporal_ablation_barplot_fixed_pca.png")
    plt.show()
    ...
    ax.axhline(0.5, color="red", linestyle="--", linewidth=1.5, label="Random guess")
    ax.set_title(f"Per-Subject AUROC — All Signals, PCs={N_PCS}", fontsize=14)
    ...
    plt.tight_layout()
    plt.savefig("temporal_per_subject_auroc_fixed_pca.png", dpi=150)
    print("Saved: temporal_per_subject_auroc_fixed_pca.png")
    plt.show()

    # =============================================================================
    # Save Final Results

    fold_metrics_all.to_csv("temporal_loso_per_subject_metrics_fixed_pca.csv", index=False)
    preds_all.to_csv("temporal_loso_all_predictions_fixed_pca.csv", index=False)

    print("\nSaved: temporal_loso_per_subject_metrics_fixed_pca.csv")
    print("Saved: temporal_loso_all_predictions_fixed_pca.csv")
